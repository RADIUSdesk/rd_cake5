...
$apStations = []; //Global array for bulk processing
$nodeStations = [];
$nodeIbss = [];
...
function _doReports(){
    global $conn, $MeshMacLookup, $counter, $ap_counter, $apStations, $nodeStations, $nodeIbss;
    ...
    if (!empty($nodeStations)) {
        insertBatchClickHouse($nodeStations,'node_stations');
    }  
    if (!empty($nodeIbss)) {
        insertBatchClickHouse($nodeIbss,'node_ibss_connections');
    }
    ...
    if (!empty($apStations)) {
        insertBatchClickHouse($apStations,'ap_stations');
    }
...
function _node_stations($d,$node_id){
    ...
    global $conn,$nodeStations,$nodeIbss;
                ...
                if(!empty($interface['stations'])){
                    if (($interface['type'] == 'AP')&&($interface['mesh_entry_id'] !== 0)) {              
                        foreach(array_keys($interface['stations']) as $s_mac){ //Only if there are any stations listed
                            ...
                            $nodeStations[] = [
                                $s_data['node_id'],//1
                                $s_data['radio_number'],//2
                                $s_data['frequency_band'],//3
                                $s_data['mesh_entry_id'],//4
                                $s_data['mac_address_id'],//5
                                $s_data['rx_bytes'],//6
                                $s_data['tx_bytes'],//7
                                $s_data['rx_bitrate'],//8
                                $s_data['tx_bitrate'],//9
                                $s_data['rx_packets'],//10
                                $s_data['tx_packets'],//11
                                $s_data['tx_failed'],//16
                                $s_data['tx_retries'],//18
                                $s_data['signal_now'],//20
                                $s_data['signal_avg'],//21,
                                $s_data['created'],//22
                            ];
                    ...
                    if (($interface['type'] == 'IBSS')||($interface['type'] == 'mesh point')) {
                        ...
                        foreach(array_keys($interface['stations']) as $s_mac){ //Only if there are any stations listed
                            ...
                            $nodeIbss[] = [
                                ...
                                $s_data['tx_packets'],//11
                                $s_data['tx_failed'],//12
                                $s_data['tx_retries'],//13
                                $s_data['signal_now'],//14
                                $s_data['signal_avg'],//15
                                $s_data['created'],//16
                            ];                                                    
    ...
    foreach(array_keys($d['radios']) as $radio) {
        if (array_key_exists('interfaces', $d['radios'][$radio])){
            foreach($d['radios'][$radio]['interfaces'] as $interface){ 
                if(!empty($interface['stations'])){
                    if (($interface['type'] == 'AP') && ($interface['mesh_entry_id'] !== 0)) {                                     
                        foreach(array_keys($interface['stations']) as $s_mac){
                            $s_data = $interface;
                            $s_data['radio_number'] = $radio;
                            unset($s_data['mac']);
                            unset($s_data['stations']);                            
                            ...
                            $apStations[] = [
                                $s_data['node_id'],
                                $s_data['radio_number'],
                                $s_data['frequency_band'],
                                $s_data['mesh_entry_id'],
                                $s_data['mac_address_id'],
                                ...
                                $s_data['tx_mcs'] ?? null,
                                $s_data['tx_nss'] ?? null,
                                $s_data['tx_short_gi'] ?? null,
                                $s_data['tx_phy'],
                                $s_data['tx_mhz'] ?? null,
                                
                                $s_data['rx_mcs'] ?? null,
                                $s_data['rx_short_gi'] ?? null,
                                $s_data['rx_phy'],
                                $s_data['rx_mhz'] ?? null,
                                
                                $s_data['noise'] ?? null,
                                $s_data['connected_time'] ?? null,
                                $s_data['vlan'] ?? null,
                                $s_data['wme'] ?? null,
                                $s_data['mfp'] ?? null,
                                $s_data['tdls'] ?? null,
...
function insertBatchClickHouse($data,$table = 'ap_stations') {
    ...
    $col_options['ap_stations'] = [
        'ap_id',            'radio_number',     'frequency_band',   'ap_profile_entry_id',
        'mac_address_id',   'tx_bytes',         'rx_bytes',         'tx_bitrate',
        'rx_bitrate',       'tx_packets',       'rx_packets',       'tx_failed',
        'tx_retries',       'signal_now',       'signal_avg',       'tx_mcs',
        'tx_nss',           'tx_short_gi',      'tx_phy',           'tx_mhz',
        'rx_mcs',           'rx_short_gi',      'rx_phy',           'rx_mhz',
        'noise',            'connected_time',   'vlan',             'wme',
        'mfp',              'tdls',             'created'
    ];
    
    //-- Nodes rx and tx order swapped from AP
    $col_options['node_stations'] = [
        'node_id',          'radio_number',     'frequency_band',   'mesh_entry_id',
        'mac_address_id',   'rx_bytes',         'tx_bytes',         'rx_bitrate',
        'tx_bitrate',       'rx_packets',       'tx_packets',       'tx_failed',
        'tx_retries',       'signal_now',       'signal_avg',       'created'
    ];
       
    $col_options['node_ibss_connections'] = [  
        'node_id',          'radio_number',     'frequency_band',   'if_mac',
        'mac',              'rx_bytes',         'tx_bytes',         'rx_bitrate',
        'tx_bitrate',       'rx_packets',       'tx_packets',       'tx_failed',
        'tx_retries',       'signal_now',       'signal_avg',       'created'
    ];
