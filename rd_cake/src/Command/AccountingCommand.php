...
use App\Model\Table\NewAccountingsTable;
...
class AccountingCommand extends Command
{
    ...
    protected CountersCommand $Counters;
    
    protected NewAccountingsTable $NewAccountings;
    ...
    protected MacUsagesTable $MacUsages;
    
    public function initialize():void{
        parent::initialize();
        
        $this->NewAccountings   = $this->fetchTable('NewAccountings');
        ...
        $this->MacUsages        = $this->fetchTable('MacUsages');
        
        // Instantate the utility classes directly to call helper methods 
        $this->Usage = new UsageCommand();
        $this->Usage->initialize();
        
        $this->Counters = new CountersCommand();
    ...
    public function execute(Arguments $args, ConsoleIo $io): int
    {
     ...
     private function process_username($username,$mac,ConsoleIo $io){
        ...
        if($type == 'voucher'){
            ...
            if($profile){
                $counters = $this->Counters->return_counter_data($profile,$type,$username,$io);
                //print_r($counters);
        ...
        if($type == 'user'){
            $io->info("$username is a Permanent User");
            //Find the profile
            $profile = $this->_find_user_profile($username);
            if($profile){
                $counters = $this->Counters->return_counter_data($profile,$type,$username,$io);
