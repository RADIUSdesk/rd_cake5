...
class CloudsController extends AppController {
    ...
    protected $ap_user_total;
    protected $ap_data_total;
    protected $ap_data_in;
    protected $ap_data_out;
        
    protected $mesh_user_total;
    protected $mesh_data_total;
    protected $mesh_data_in;
    protected $mesh_data_out;
    
    protected $graph_item;
        ...
        if(isset($req_q['node'])){
            ...
            if($level == 'MarkerNetworks'){
                ...
                if($network){
                    ...
                    $cut_off_time   = $now->subSeconds($dead_after);
    ...
    public function index(){
        ...
        $q_r    = $query->orderBy([$l_model.'.name' => 'ASC'])->all();
        $total  = $query->count();
    ...
    private function _isSomethingDown($level,$id){
        ...
        $now            = DateTime::now();
        $dead_after     = $this->_getDefaultDeadAfter();      
        $cut_off        = $now->subSeconds($dead_after);       
    ...
    private function _infoBuilding(){    
        ...
        $ft_day = DateTime::now();
        $cut_off = $ft_day->subDays(1);
        
        if($this->timespan == 'now'){
            $dead_after = $this->_getDefaultDeadAfter();      
            $cut_off    = $ft_day->subSeconds($dead_after);
        }  
        ...
        if($this->timespan == 'weekly'){
            $cut_off = $ft_day->subDays(7);
        }
        if($this->timespan == 'monthly'){
            $cut_off = $ft_day->subDays(30);
        }
