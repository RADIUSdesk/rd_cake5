...
class OtpComponent extends Component {
    ...
    protected array $components = ['MailTransport','RdLogger'];
         
    public function sendEmailClientReg($email_adr,$otp,$data_id){  
    	...
    	$url 	= $protocol.$this->getController()->getRequest()->host()."/cake4/rd_cake/register-clients/otp-confirm?data_id=$data_id&otp=$otp"; 
                           
        if($meta_data !== false){         
            $email = new Mailer(['transport'   => 'mail_rd']);
            $from  = $meta_data['from'];
            $email->setFrom($from)
            	->setSubject(__("Client registration OTP"))
            	->setTo($email_adr)
            	->setViewVars(compact( 'username', 'otp','url'))
            	->setViewVars(compact( 'username', 'otp'))
            	->setEmailFormat('html')
             	->viewBuilder()
                    	->setTemplate('otp_client')
