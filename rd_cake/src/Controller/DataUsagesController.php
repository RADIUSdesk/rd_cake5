...
class DataUsagesController extends AppController {
   ...
   // protected   $time_zone            = 'Africa/Johannesburg';
    ...
    public function usageForRealm() {
        ...
        if($e_us){
            $this->dailies_stopped = DateTime::createFromTimestamp($e_us->value);
        }
        ...
        $historical     = true;  
        $tz_adjusted    = DateTime::createFromTimestamp($ft_day->created,$this->time_zone);
        ...
        if($this->type == 'realm'){ 
            if($historical == false){ //Only when its live data
                ...
                $this->RadacctHistories       = $this->fetchTable('RadacctHistories');
    ...
    public function macsForUser(){
        ...
        if($span == 'month'){  
            ...
            if($this->data_limit_active){ 
                //FIXME For later          
                $slot_start = DateTime::createFromTimestamp($this->_start_of_month($this->start_of_month,$this->start_hour,$this->start_minute));
                $end_time   = $slot_start; 
                $slot_end   = $end_time->addMonths(1)->subMinutes(1); 
            }else{
            ...
            if($this->dailies_stopped ){
                if(($slot_start < $this->dailies_stopped)&&($slot_start->addMonths(1)->subSeconds(1)>$this->dailies_stopped)){
                    $table      = 'UserStats';                      
                }
            }   
        }
            ...
            if($this->data_limit_active){ 
                ...
                if($this->dailies_stopped ){
                    if(($slot_start < $this->dailies_stopped)&&($slot_start->addMonths(1)->subSeconds(1)>$this->dailies_stopped)){
                        ...
                        $daily_slot_end_txt     = $this->dailies_stopped->i18nFormat('yyyy-MM-dd HH:mm:ss');
                        $slot_start_txt         = $this->dailies_stopped->i18nFormat('yyyy-MM-dd HH:mm:ss');
                        $slot_end_txt           = $slot_start->addMonths(1)->subSeconds(1)->i18nFormat('yyyy-MM-dd HH:mm:ss');
                    }
                    
                    if($slot_start->addMonths(1)->subSeconds(1) < $this->dailies_stopped){
                        $table              = 'UserStatsDailies';
                        $slot_start_txt     = $slot_start->i18nFormat('yyyy-MM-dd HH:mm:ss');
                        $slot_end_txt       = $slot_start->addMonths(1)->subSeconds(1)->i18nFormat('yyyy-MM-dd HH:mm:ss');      
                    }                    
                ...
                if($this->dailies_stopped ){
                    ...
                    if((!$this->dailies_stopped->subSeconds(1)->isThisMonth())&&($ft_day->endOfMonth() > $this->dailies_stopped)){ 
                        $table          = 'UserStats';
                        $slot_start_txt = $ft_day->startOfMonth()->i18nFormat('yyyy-MM-dd HH:mm:ss');
                        $slot_end_txt   = $ft_day->endOfMonth()->i18nFormat('yyyy-MM-dd HH:mm:ss');            
                    }     
            ...
            if($this->data_limit_active){ 
                ...
                if($this->dailies_stopped ){
                    if(($slot_start < $this->dailies_stopped)&&($slot_start->addMonths(1)->subSeconds(1)>$this->dailies_stopped)){
                        ...
                        $daily_slot_end     = $this->dailies_stopped->i18nFormat('yyyy-MM-dd HH:mm:ss');
                        $slot_start         = $this->dailies_stopped->i18nFormat('yyyy-MM-dd HH:mm:ss');
                        $slot_end           = $slot_start->addMonths(1)->subSeconds(1)->i18nFormat('yyyy-MM-dd HH:mm:ss');
                    }
    ...
    private function _start_of_month($ft_day,$r_day,$r_hour,$r_min){  
        #Get the day of the month for this date-time selection
