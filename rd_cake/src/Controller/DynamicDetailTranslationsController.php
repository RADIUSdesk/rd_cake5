...
use App\Model\Table\DynamicDetailTranslationsTable;
use App\Model\Table\DynamicDetailLanguagesTable;
use App\Model\Table\DynamicDetailTransKeysTable;
use App\Model\Table\DynamicDetailsTable;
use App\Model\Table\DynamicPhotoTranslationsTable;
...
class DynamicDetailTranslationsController extends AppController {
    ...
    protected DynamicDetailTranslationsTable $DynamicDetailTranslations;
    protected DynamicDetailLanguagesTable $DynamicDetailLanguages;
    protected DynamicDetailTransKeysTable $DynamicDetailTransKeys;
    protected DynamicDetailsTable $DynamicDetails;
    protected DynamicPhotoTranslationsTable $DynamicPhotoTranslations;
    ...
    public function initialize():void{
        ...
        $this->DynamicDetailTranslations    = $this->fetchTable('DynamicDetailTranslations');
        $this->DynamicDetailLanguages       = $this->fetchTable('DynamicDetailLanguages');
        $this->DynamicDetailTransKeys       = $this->fetchTable('DynamicDetailTransKeys');
        $this->DynamicDetails               = $this->fetchTable('DynamicDetails');
        $this->DynamicPhotoTranslations     = $this->fetchTable('DynamicPhotoTranslations');
    ...
    public function getTranslations(){
        ...
        $query  = $this->{$this->main_model}->find()
                    ...
                    ->orderBy(['DynamicDetailTransKeys.name']);
        $q_r    = $query->all();
        foreach ($q_r as $i) {
            $value = $i->value;
    ...
    public function index(){
        ...
        $user_id    = $user['id'];
        $dd_id      = $this->request->getQuery('dynamic_detail_id');   
        $query      = $this->{$this->main_model}->find()->contain(['DynamicDetailTransKeys','DynamicDetailLanguages'])->where(['DynamicDetailTransKeys.dynamic_detail_id' => $dd_id])->orderBy(['DynamicDetailTransKeys.name']);
        $q_r        = $query->all();
        $items      = [];
        foreach ($q_r as $i) {           
            $row        = [];
    ...
    public function languagesList(){  
        $q = $this->{'DynamicDetailLanguages'}->find()->orderBy(['DynamicDetailLanguages.name'])->all();
        $items = [];
        foreach($q as $i){
            array_push($items, $i);
        }
    ...
    public function pagesList(){
        ...
        $query->orderBy(['DynamicDetails.name']);
        $q_r   = $query->all();   
        $items = [];
    ...
    public function keysList(){ 
        $dd_id  = $this->request->getQuery('dynamic_detail_id');
        $q      = $this->{'DynamicDetailTransKeys'}->find()->where(['dynamic_detail_id' => $dd_id])->orderBy(['DynamicDetailTransKeys.name'])->all();
        $items  = [];
        foreach($q as $i){
            array_push($items, $i);
        }
    ...
    public function delPhrase(){
	    ...
	    if(isset($req_d['id'])){   //Single item delete
            ...
            $entity  = $this->{'DynamicDetailTranslations'}->get($req_d['id']);   
    ...
    public function saveTweaks(){
        ...
        if (array_key_exists('available_languages', $this->request->getData())) {                   
            foreach($this->request->getData('available_languages') as $lang_id){
