...
class FreeRadiusController extends AppController {
    ...
    protected $radscenario      = '/var/www/html/cake4/rd_cake/setup/scripts/radscenario.pl';
    
    protected ChecksTable $Checks;
    ...
    //== Only Root ==
        ...
        if((isset($cdata['nas_id'])) && (null !== $cdata['username'])){
        ...
        if(($cdata['username']) && (null !== $cdata['nas_id'])){
            $username = $cdata['username'];
            exec("sudo " . $this->radmin_wrapper . " debug condition '(User-Name == $username)'",$output);
        }

        if((isset($cdata['username'])) && (isset($cdata['nas_id']))){
