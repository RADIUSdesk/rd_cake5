...
use App\Model\Table\MeshExitsTable;
use App\Model\Table\OpenvpnServerClientsTable;
...
class MeshReportsController extends AppController {
    ...
    protected MeshExitsTable $MeshExits;
    protected OpenvpnServerClientsTable $OpenvpnServerClients;
    protected MacActionsTable $MacActions;
    protected MacAddressesTable $MacAddresses;
    protected HardwaresTable $Hardwares;
    ...
    public function initialize():void
    {
        ...
        $this->MeshExits            = $this->fetchTable('MeshExits');
        $this->OpenvpnServerClients = $this->fetchTable('OpenvpnServerClients');
            ...
            if ($q_s) {
                foreach ($q_s as $j) {
                    ...
                    $lastCreated = $this->NodeIbssConnections->find()->where([
                        'NodeIbssConnections.mac'       => $mac,
                        'NodeIbssConnections.node_id'   => $node_id
                    ])->orderBy(['NodeIbssConnections.created' => 'desc'])->first();
                ...
                if($this->{'Nodes'}->save($ent_node)){
                    if ($cfg['enable_realtime']){
                        ...
                        if ($this->_check_server($client, $cfg['api_gateway_url'], 5)) {
                            try {
                                $client->post(
                                    $cfg['api_gateway_url'] . '/rd/mesh/command',
                                    ['message' => $payload],
                                    ['type' => 'json']
                                );
                            } catch (\Exception $e) {
