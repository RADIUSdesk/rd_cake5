...
use App\Model\Table\MeshEntrySchedulesTable;
...
class MeshesController extends AppController{
    ...
    protected MeshEntrySchedulesTable $MeshEntrySchedules;
    ...
    public function initialize():void{  
        ...
        $this->MeshEntrySchedules      = $this->fetchTable('MeshEntrySchedules');
    ...
    public function index(){  
        ...
        $items  = [];
        $ft_now = DateTime::now();
        $ft_dead= $ft_now->subSeconds($this->dead_after);
	...
	public function meshSettingsView(){
        ...
        $this->MeshSettings       = $this->fetchTable('MeshSettings');
    ...
    public function meshExitAdd(){
        ...
        if ($this->{'MeshExits'}->save($entity)) {
            ...
            if($this->request->getData('type') == 'nat'){;
                $this->MeshExitSettings       = $this->fetchTable('MeshExitSettings');
            ...
            if($this->request->getData('type') == 'pppoe_server'){
                $this->MeshExitPppoeServers       = $this->fetchTable('MeshExitPppoeServers');
                $d_ppp  = [
                   'mesh_exit_id'       => $new_id,
    ...
    public function meshExitEdit(){
        ...
        if ($this->request->is('post')) {
            ...
            if($req_d['type'] == 'openvpn_bridge'){
                ...
                $this->OpenvpnServers       = $this->fetchTable('OpenvpnServers');
                $this->OpenvpnClients       = $this->fetchTable('OpenvpnClients');
                $this->OpenvpnServerClients = $this->fetchTable('OpenvpnServerClients');
...
//================PORT MAPS=============
	public function mapPrefView(){
		...
		$this->MeshSettings = $this->fetchTable('MeshSettings');
        $this->MeshSpecifics= $this->fetchTable('MeshSpecifics');
        $this->UserSettings = $this->fetchTable('UserSettings');
...
//===========END PORT MAPS====================
    ...
    public function meshNodeAdd(){
        ...
        $this->NodeMeshEntries         = $this->fetchTable('NodeMeshEntries');
        $this->Nodes                   = $this->fetchTable('Nodes');
        $this->NodeWifiSettings        = $this->fetchTable('NodeWifiSettings');
