...
use App\Model\Table\MeshesTable;
use App\Model\Table\NodesTable;
use App\Model\Table\NodeLoadsTable;
use App\Model\Table\NodeActionsTable;
use App\Model\Table\NodeSystemsTable;
use App\Model\Table\NodeSettingsTable;
use App\Model\Table\NodeScansTable;
use App\Model\Table\MeshEntriesTable;
use App\Model\Table\NodeIbssConnectionsTable;
use App\Model\Table\NodeStationsTable;
use App\Model\Table\NodeNeighborsTable;
use App\Model\Table\NodeUptmHistoriesTable;
use App\Model\Table\StagingStationsTable;
...
class NodeReportsController extends AppController {
    ...
    private $repSettings        = [];
    protected MeshesTable $Meshes;
    protected NodesTable $Nodes;
    protected NodeLoadsTable $NodeLoads;
    protected NodeActionsTable $NodeActions;
    protected NodeSystemsTable $NodeSystems;
    protected NodeSettingsTable $NodeSettings;
    protected NodeScansTable $NodeScans;
    protected MeshEntriesTable $MeshEntries;
    protected NodeIbssConnectionsTable $NodeIbssConnections;
    protected NodeStationsTable $NodeStations;
    protected NodeNeighborsTable $NodeNeighbors;
    protected NodeUptmHistoriesTable $NodeUptmHistories;
    protected StagingStationsTable $StagingStations;
      

    public function initialize(){
        parent::initialize();
        
        $this->Meshes                = $this->fetchTable('Meshes');
        $this->Nodes                 = $this->fetchTable('Nodes');
        $this->NodeLoads             = $this->fetchTable('NodeLoads');
        $this->NodeActions           = $this->fetchTable('NodeActions');
        $this->NodeSystems           = $this->fetchTable('NodeSystems');
        $this->NodeSettings          = $this->fetchTable('NodeSettings');
        $this->NodeScans             = $this->fetchTable('NodeScans');
        $this->MeshEntries           = $this->fetchTable('MeshEntries');
        $this->NodeIbssConnections   = $this->fetchTable('NodeIbssConnections');
        $this->NodeStations          = $this->fetchTable('NodeStations');
        $this->NodeNeighbors         = $this->fetchTable('NodeNeighbors');
        $this->NodeUptmHistories     = $this->fetchTable('NodeUptmHistories');
        $this->StagingStations       = $this->fetchTable('StagingStations');
    ...
    public function submitRogueReport(){
        ...
        if(($mac)&&($mode)){
            $data               = [];    
            $data['scan_data']  = json_encode($req_d['scan_data']);
    ...
    public function startScanForNode(){
		...
		if(isset($req_d['node_id'])){
		    $node_id = $req_d['node_id'];
    ...
    public function submitReport(){
        ...
        if(isset($req_d['report_type'])){
            $fb = [];
            if($req_d['report_type'] == 'light'){
                $fb = $this->_light_report(); 
            } 
            
            if($req_d['report_type'] == 'full'){
    ...
    public function _node_stations($e){
        ...
        $req_d	= $this->request->getData();
        $d_now  = new DateTime($req_d['unix_timestamp']);
                    ...
                    if(!empty($interface['stations'])){
                        if (($interface['type'] == 'AP')&&($interface['mesh_entry_id'] !== 0)) {              
                            foreach(array_keys($interface['stations']) as $s_mac){ //Only if there are any stations listed
                                ...
                                $s_data['created']          = new DateTime($s_data['first_timestamp']+($drift));
                                $s_data['modified']         = new DateTime($s_data['unix_timestamp']+($drift)); 
    ...
    private function _addWbwInfo($node_id){ 
    	...
    	$req_d	= $this->request->getData();  
        $this->NodeConnectionSettings    = $this->fetchTable('NodeConnectionSettings');
        $wbw_info = $req_d['wbw_info'];
    ...
    private function _full_report(){
        ...
        if($entity){
            ...
            //Update the Loads
            $this->_do_node_load($entity->id,$req_d['system_info']['sys']);
            //Update system info (if required)
            $this->_do_node_system_info($entity->id,$req_d['system_info']['sys']);
    ...
    private function _rebootFlagTest($mac){
        ...
        $this->UnknownNodes    = $this->fetchTable('UnknownNodes');
    ...
    private function _new_report(){
        ...
        if($entity){
            ...
            if (array_key_exists(0, $req_d['network_info']['radios'])) {
                $r0 = $req_d['network_info']['radios'][0]['interfaces'];
                $this->_do_radio_interfaces($entity, $r0);
            }

            //If it is a dual radio --- report on it also ----
            if (array_key_exists(1, $req_d['network_info']['radios'])) {
                $r1 = $req_d['network_info']['radios'][1]['interfaces'];
    ...
    private function _new_do_uptm_history($entity){
        ...
        $e_uptm = $this->{'NodeUptmHistories'}->find()
                ...
                ->orderBy(['NodeUptmHistories.report_datetime' => 'desc'])
                ->first();
    ...
    private function _do_uptm_history($entity){
        ...
        if (array_key_exists('bootcycle', $req_d)) {
            $bootcycle  = intval($req_d['bootcycle']); 
        }else{
            return; 
        }
    ...
    private function _do_vis(){
        ...
        if (array_key_exists('vis', $req_d)) {     
            foreach ($req_d['vis'] as $vis) {
                $metric         = $vis['metric'];
 ...
 //   	return;
                    ...
                    if ($entity) {
                        ...
                        foreach ($i['stations'] as $s) {
                            ...
                            $entity_mac = $this->{'NodeStations'}->find()
                                ...
                                ->orderBy(['NodeStations.created' => 'desc'])
                                ->first();     
                            $new_flag = true;
                ...
                if (($i['type'] == 'IBSS')||($i['type'] == 'mesh point')) {
                    ...
                    foreach ($i['stations'] as $s) {
                        ...
                        $entity_mac = $this->{'NodeIbssConnections'}->find()
                            ...
                            ->orderBy(['NodeIbssConnections.created' => 'desc'])
                            ->first();
    ...
    private function _update_last_contact($e){
        ...
        if (array_key_exists('lan_info', $req_d)) {
            $lan_proto  = $req_d['lan_info']['lan_proto'];
            $lan_gw     = $req_d['lan_info']['lan_gw'];
            $lan_ip     = $req_d['lan_info']['lan_ip'];
        ...
        if (array_key_exists('bootcycle', $req_d)) {
            $entity->bootcycle  = intval($req_d['bootcycle']); 
        }
        
        //--Check if we need to update the gateway field
        if (array_key_exists('gateway', $req_d)){
            if($e->gateway !== $req_d['gateway']){
                $entity->gateway = $req_d['gateway']; //Set the gateway only if it is set and different
            }
        }
