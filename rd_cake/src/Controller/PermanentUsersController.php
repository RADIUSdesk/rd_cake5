...
use App\Model\Table\RealmVlansTable;
...
class PermanentUsersController extends AppController{
    ...
    protected RealmVlansTable $RealmVlans;
    ...
    public function initialize():void{  
        ...
        $this->RealmVlans       = $this->fetchTable('RealmVlans');  
            ...
            if(isset($req_q['columns'])){
                ...
                foreach($columns as $c){
                    ...
                    if($column_name == 'cleartext_password'){
                        ...
                        $last_session = $this->{'Radaccts'}->find()->where(['username' => $i->username])->select(['acctstarttime','acctstoptime','framedipaddress'])->orderBy('acctstarttime DESC')->first();
                        if($last_session){
    ...
    public function index(){
        ...
        foreach($q_r as $i){
            ...
            //select acctstarttime,acctstoptime,framedipaddress from radacct where username='ord9555@superfibre' orderBy by acctstarttime DESC LIMIT 1;          
            $last_session = $this->{'Radaccts'}->find()->where(['username' => $i->username])->select(['acctstarttime','acctstoptime','framedipaddress'])->orderBy('acctstarttime DESC')->first();
            if ($last_session) {
    ...
    public function add(){
        ...
        if(isset($req_d['from_date'])){
            if($req_d['from_date'] === 'now'){ //Special keyword for API
                $req_d['from_date'] = DateTime::now()>startOfDay();
            }else{
                $from = new DateTime($req_d['from_date']);
        ...
        if(isset($req_d['to_date'])){
            if (filter_var($req_d['to_date'], FILTER_VALIDATE_INT) !== false && $req_d['to_date'] < 100) { //Special type for API (Integer = times 30)
                $to = DateTime::now()->endOfDay();
                $to = $to->addDays(($req_d['to_date']*30));               
                $req_d['to_date'] = $to;
            }else{
            
                $to = new DateTime($req_d['to_date']);
                $to = $to->endOfDay();    
                $req_d['to_date'] = $to;
            }
        }
        ...
        if($req_d['always_active'] == 0){
            ...
            if(isset($req_d['to_date'])){
                if (filter_var($req_d['to_date'], FILTER_VALIDATE_INT) !== false && $req_d['to_date'] < 100) { //Special type for API (Integer = times 30)
                    ...
                    $oldToDate = $entity->to_date instanceof DateTime
                        ? $entity->to_date
                        : DateTime::parse($entity->to_date);

                    $now = DateTime::now()->endOfDay();
