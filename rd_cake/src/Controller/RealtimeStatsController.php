...
use App\Model\Table\DynamicClientSettingsTable;
use App\Model\Table\MikrotikPppoeStatsTable;
...
class RealtimeStatsController extends AppController{
    ...
    protected DynamicClientSettingsTable $DynamicClientSettings;
    protected MikrotikPppoeStatsTable $MikrotikPppoeStats;
    
    public function initialize():void{  
        parent::initialize();
        
        $this->DynamicClientSettings    = $this->fetchTable('DynamicClientSettings');
        $this->MikrotikPppoeStats       = $this->fetchTable('MikrotikPppoeStats');
    ...
    private function _getChartData($user_id){
        ...
        $ft_now     = DateTime::now();
        $slot_start = $ft_now->subSeconds($this->span);        
        $previous   = null;
        
        while($slot_start < $ft_now){
            $slot_start_h_m = $slot_start->i18nFormat("E\nHH:mm:ss",$this->time_zone);
            $slot_end       = $slot_start->addSeconds($this->interval)->subSeconds(1);
            ...
            $slot_start = $slot_start->addSeconds($this->interval);
