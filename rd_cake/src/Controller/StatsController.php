...
class StatsController extends AppController{
    ...
    public function renewalsByCloud(){
        ...
        foreach($clouds as $cloud){
            $cloud_data[$cloud->id] = [
                ...
                'new_registrations'      => $this->_newRegistrations($span,$cloud->id)
            ];
        }
    ...
    public function usersByCloud(){
        ...
        $userStats = $permanentUsersTable->find()
            ...
            ->groupBy(['PermanentUsers.cloud_id', 'PermanentUsers.admin_state'])
            ->orderBy(['PermanentUsers.cloud_id' => 'ASC'])
            ->toArray();
    ...
    private function _newRegistrations($days,$cloud_id){
    
        $startDate = DateTime::now('Africa/Lagos')->startOfDay();

        if($days > 1){
            $days = $days -1;
            $startDate = DateTime::now('Africa/Lagos')->subDays($days)->startOfDay();
        }
    
        $count = $this->PermanentUsers->find()->where([
            'PermanentUsers.cloud_id' => $cloud_id,
            'PermanentUsers.created >=' => $startDate
        ])->count();
