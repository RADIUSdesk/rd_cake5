...
class ThirdPartyAuthsController extends AppController{
    ...
    public function index(){
        ...
        if(!isset($req_q['idp_name'])){
            $this->JsonErrors->errorMessage('No IDP name specified');
            return;
        }else{
        
            if(!in_array($req_q['idp_name'],$this->idp_avail)){
    ...
    public function infoFor(){
		...
		$req_q    = $this->request->getQuery();
		if(
			(array_key_exists('sl_type',$req_q))&&
			(array_key_exists('sl_name',$req_q))&&
			(array_key_exists('sl_value',$req_q))
		){

			if($req_q['sl_type'] == 'voucher'){
    ...
    private function _determine_sl($req_q){
		...
		$q_r = $this->DynamicPairs
            ...
            ->where([$conditions])
            ->orderBy(['DynamicPairs.priority' => 'DESC'])
            ->first();
        ...
        if($q_r){
            ...
            $this->dynamic_detail_cloud_id = $q_r->dynamic_detail->cloud_id;
            
            foreach($q_r->dynamic_detail->dynamic_detail_social_logins as $ssli){
                if($ssli->name == $req_q['idp_name']){
	...
	private function _authForRadius(){
		...
		if($type == 'voucher'){
			...
			if(!$v){
				...
				$i['cloud_id'] 		= intval($this->dynamic_detail_cloud_id);
				$this->_addVoucher($i);
			}
        }
		...
		if($this->social_login_info->record_info){
			...
			$this->{'SocialLoginUserRealms'} = TableRegistry::getTableLocator()->get('SocialLoginUserRealms');
		...
		$redirect_url       = urldecode($req_q['protocol']).'//'.
            urldecode($req_q['hostname']).
            urldecode($req_q['pathname'])."?".
            urldecode($new_query_string);
		$this->redirect("$redirect_url");       
	}
	
	private function _addVoucher($i){
		$url            = 'http://127.0.0.1/cake4/rd_cake/vouchers/add.json';
	...
	private function _record_or_update_info(){
		...
		$this->SocialLoginUsers = TableRegistry::getTableLocator()->get('SocialLoginUsers');
