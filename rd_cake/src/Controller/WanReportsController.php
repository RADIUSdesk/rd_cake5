...
use App\Model\Table\WanWifiStatsTable;
use App\Model\Table\WanLteStatsTable;
use App\Model\Table\WanTrafficStatsTable;
use App\Model\Table\WanMwanStatusTable;
...
class WanReportsController extends AppController {
    ...
    protected WanWifiStatsTable $WanWifiStats;
    protected WanLteStatsTable $WanLteStats;
    protected WanTrafficStatsTable $WanTrafficStats;
    protected WanMwanStatusTable $WanMwanStatus;
    protected ApsTable $Aps;
    
    public function initialize():void{
        parent::initialize();
        $this->Timezones          = $this->fetchTable('Timezones');
        $this->WanWifiStats       = $this->fetchTable('WanWifiStats');
        $this->WanLteStats        = $this->fetchTable('WanLteStats');
        $this->WanTrafficStats    = $this->fetchTable('WanTrafficStats');
        $this->WanMwanStatus      = $this->fetchTable('WanMwanStatus');
    ...
    private function _getTrafficData($apId, $mwanInterfaceId, $interval, $duration){
        ...
        while ($slotStart < $currentTime) {
            $slotEnd = $slotStart->addMinutes($interval)->subSeconds(1);
