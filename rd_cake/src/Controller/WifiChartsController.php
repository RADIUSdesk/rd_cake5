...
use App\Model\Table\MacAliasesTable;
use App\Model\Table\NodeStationsTable;
use App\Model\Table\ApStationsTable;
use App\Model\Table\ApsTable;
use App\Model\Table\NodeStationHourliesTable;
use App\Model\Table\ApStationHourliesTable;
use App\Model\Table\MacActionsTable;
use App\Model\Table\MacAddressesTable;
use App\Model\Table\NodeActionsTable;
use App\Model\Table\ApActionsTable;
...
class WifiChartsController extends AppController{
    ...
    protected $base_search;
    protected $base_search_no_mac;
    ...
    protected ApActionsTable $ApActions;
    protected ApProfilesTable $ApProfiles;
    protected ApsTable $Aps;
    protected ApStationHourliesTable $ApStationHourlies;
    protected ApStationsTable $ApStations;
    protected MacActionsTable $MacActions;
    protected MacAddressesTable $MacAddresses;
    protected MacAliasesTable $MacAliases;
    protected MeshesTable $Meshes;
    protected NodeActionsTable $NodeActions;
    protected NodesTable $Nodes;
    protected NodeStationHourliesTable $NodeStationHourlies;
    ...
    public function initialize():void{  
        ...
        $this->ApActions             = $this->fetchTable('ApActions');
        $this->ApProfiles            = $this->fetchTable('ApProfiles');
        $this->Aps                   = $this->fetchTable('Aps');
        $this->ApStationHourlies     = $this->fetchTable('ApStationHourlies');
        $this->ApStations            = $this->fetchTable('ApStations');
        $this->MacActions            = $this->fetchTable('MacActions');
        $this->MacAddresses          = $this->fetchTable('MacAddresses');
        $this->MacAliases            = $this->fetchTable('MacAliases');
        $this->Meshes                = $this->fetchTable('Meshes');
        $this->NodeActions           = $this->fetchTable('NodeActions');
        $this->Nodes                 = $this->fetchTable('Nodes');
        $this->NodeStationHourlies   = $this->fetchTable('NodeStationHourlies');
    ...
    public function apUsageForSsid(){
        ...
        if($span == 'hour'){
            ...
            $ft_start       = $ft_now->subHours(1);
        }
        if($span == 'day'){
            $graph_items = $this->_getDailyGraph($ft_now);
            $ft_start    = $ft_now->subHours(24);
        }
        if($span == 'week'){
            $graph_items = $this->_getWeeklyGraph($ft_now);
            $ft_start    = $ft_now->subHours((24*7));
        }
    ...
    private function _getNodeData($ft_start,$ft_end){
        ...
        $nodes = $this->{$table}->find()->select($fields)
            ->where($where)
            ->orderBy(['data_total' => 'DESC'])
            ->groupBy(['node_id','Nodes.name'])
            ->contain(['Nodes'])
            ->all();
    ...
    private function _device_info(){
        ...
        $station        = $this->{"$table_raw"}->find()->where($where)->orderBy(["$table_raw.modified DESC"])->contain($contain)->first();
        ...
        if($station){
            ...
            $station_h       = $this->{"$tHourlies"}->find()->where($where)->orderBy(["$tHourlies.modified DESC"])->contain($contain)->first();
            if($station_h){
                $di = $station_h;
    ...
    private function _getHourlyGraph($ft_now){
        ...
        while($slot_start < $hour_end){
            ...
            $slot_end       = $slot_start->addMinutes(10)->subSeconds(1);  
            $where          = $base_search;
            array_push($where, ["modified >=" => $slot_start]);
            array_push($where, ["modified <=" => $slot_end]);   
            $slot_start     = $slot_start->addMinutes(10); 
    ...
    private function _getDailyGraph($ft_now){
        ...
        while($slot_start < $day_end){  
            $slot_start_h_m = $slot_start->i18nFormat("E\nHH:mm",$this->time_zone);
            $slot_end       = $slot_start->addHours(1)->subSeconds(1);   
            $where          = $base_search;
            array_push($where, ["modified >=" => $slot_start]);
            array_push($where, ["modified <=" => $slot_end]);    
     ...
     private function _getWeeklyGraph($ft_day){
        ...
        $base_search    = $this->base_search;       
        $week_end       = $ft_day;         
        $slot_start     = $ft_day->subHours(24*7)->hour(00)->minute(00)->second(00);
        $table          = 'NodeStations';
       ...
       // print_r($slot_start);
        ...
        while($slot_start < $week_end){
            ...
            $slot_end       = $slot_start->addDays(1)->subSeconds(1); //Our interval is one day 
            array_push($where, ["modified >=" => $slot_start]);
            array_push($where, ["modified <=" => $slot_end]);    
    ...
    private function _getTopTen($ft_start,$ft_end){
        ...
        $stations = $this->{$table}->find()->select($fields)
            ->where($where)
            ->orderBy(['data_total' => 'DESC'])
            ->groupBy(['mac_address_id', 'MacAddresses.mac'])
